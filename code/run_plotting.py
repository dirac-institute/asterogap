...
def set_kernel(data, time, flux, flux_err):
    """
    ...
    # k2
    log_amp_k2 = np.log(flux.max()-flux.min())
    log_gamma = 0
    log_period = 0
    ...
    k1 = log_amp_k1 * george.kernels.ExpSquaredKernel(metric=np.exp(log_metric))
    k2 = log_amp_k2 * george.kernels.ExpSine2Kernel(gamma=np.exp(log_gamma), log_period=log_period)

    print(data.shape[2])

    if data.shape[2] == 6: kernel = k1*k2
    else: kernel = k2


    gp = george.GP(kernel, mean=mean_flux, fit_mean=True)
...
def plot_corner(data, true_period=None, colours=None, zoom=False, trim=None, fig=None):
    ...
    # new_gamma = np.random.random(flat_data.shape[0])*100
    # print(new_gamma)
    # print(new_gamma.shape)
    #
    # print(flat_data.T[-2])
    # print(flat_data.T[-2].shape)
    #
    # print(flat_data.shape)
    #
    # flat_data.T[-2] = new_gamma


    figure = corner.corner(flat_data, labels=labels, truths=truths,
...
def plot_trace(data, iterations, colours=None):
    ...
    fig, ax = plt.subplots(2, int(data.shape[2]/2), figsize=(5*int(data.shape[2]/2),9))
...
def make_summary_plots(filename, save_fig = False, true_period=None, true_lightcurve=None,):
    """
    ...
    with h5py.File(filename, "r") as f:
        ...
        if save_fig: plt.savefig(filename.replace(".hdf5", "_lsp.pdf"), format="pdf")


        ###   TRACE PLOT   ###
        ### should be fully functional in both 4 and 6 dim, with period and without
        plot_trace(data, f.attrs['iterations'])
        if save_fig: plt.savefig(filename.replace(".hdf5", "_trace.pdf"), format="pdf")


        ###   CORNER PLOTS   ###
        ### should be fully functional in both 4 and 6 dim, with period and without
        plot_corner(data, true_period)
        if save_fig: plt.savefig(filename.replace(".hdf5", "_corner.pdf"), format="pdf")

        plot_corner(data, true_period, trim=[5,95])
        if save_fig: plt.savefig(filename.replace(".hdf5", "_corner_5_95.pdf"), format="pdf")

        plot_corner(data, true_period, zoom=True)
        if save_fig: plt.savefig(filename.replace(".hdf5", "_corner_zoom.pdf"), format="pdf")


        ###   POSTERIOR   ###
        ### should be fully functional in both 4 and 6 dim, with period and without
        best_period = plot_posterior(data, true_period)
        if save_fig: plt.savefig(filename.replace(".hdf5", "_posterior.pdf"), format="pdf")

        # ###   FOLDED LIGHTCURVE   ###
        ### should be fully functional in both 4 and 6 dim, with period and without
        plot_folded_lightcurve(time, flux, flux_err=flux_err, legend=False,
                                            period=best_period, true_lightcurve=true_lightcurve)
        if save_fig: plt.savefig(filename.replace(".hdf5", "_folded.pdf"), format="pdf")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--save_fig', action="store_true", dest="save_fig", required=False, default=False,
                        help="Sets to true if you want to save the figures generated.")
    parser.add_argument('-p', '--period', action="store", dest="period", required=False, type=float,
